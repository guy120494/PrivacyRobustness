...
class NeuralNetwork(nn.Module):
    def __init__(self, input_dim, hidden_dim_list, output_dim, activation, use_bias=False):
        ...
        self.layers = nn.ModuleList([nn.Linear(input_dim, hidden_dim_list[0], bias=use_bias)])
        for i in range(1, len(hidden_dim_list)):
            self.layers.append(nn.Linear(hidden_dim_list[i-1], hidden_dim_list[i], bias=False))
